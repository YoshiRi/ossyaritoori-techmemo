...
class TempTracker:
    ...
    def match(self,img_c):
        if len(img_c.shape) > 2: #if color then convert BGR to GRAY
            img = cv2.cvtColor(img_c, cv2.COLOR_BGR2GRAY)
             
        self.kp2,des2 = self.detector.detectAndCompute(img,None)
        print(len(self.kp2))
        if len(self.kp2) < 5:
            return
            
        self.matchings = self.bf.knnMatch(self.des1,des2,k=2)
        good = []
        pts1 = []
        pts2 = []
        ...
        #matchedimg = cv2.drawMatches(self.template, self.kp1, img, kp2, good, None, flags=2)
